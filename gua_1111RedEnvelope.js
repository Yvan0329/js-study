...
function getCoupons(shareId = '', type = 1) {
    return new Promise(resolve => {
        let opts = {
            url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body={%22platform%22:4,%22unionActId%22:%2231134%22,%22actId%22:%22${$.actId}%22,%22d%22:%223wXaBoQ%22,%22unionShareId%22:%22${shareId}%22,%22type%22:${type},%22eid%22:%22${$.eid}%22}&client=apple&clientVersion=8.3.6&h5st=undefined`,
            headers: {
...
function getUrl() {
    return new Promise(resolve => {
        const options = {
            url: `https://u.jd.com/3wXaBoQ?s=${$.shareCode}`,
            followRedirect: false,
            headers: {
                'Cookie': `${cookie} ${newCookie}`,
                "User-Agent": $.UA
